"""Didi Food scraper for the Rappi Analytics ETL pipeline.

This module provides platform-specific scraping logic for Didi Food,
...
class DidiFoodScraper(BaseScraper):
    """Scraper implementation for Didi Food platform.

    This class implements platform-specific scraping logic for Didi Food,
    including:
    ...
    async def set_location(self, page: Page, address: str) -> bool:
        """Set delivery location on Didi Food.
        ...
        try:
            ...
            suggestion = page.locator(
                "[data-testid='suggestion'], .suggestion-item, li[class*='suggestion']"
            ).first
    ...
    async def search_restaurant(self, page: Page, restaurant_name: str) -> bool:
        ...
        try:
            ...
            search_url = discovery.url or build_didi_food_url()
            if discovery.url:
    ...
    async def extract_product_data(
        ...
        try:
            ...
            restaurant_cards = page.locator(
                "[data-testid='store-card'], [class*='RestaurantCard'], [class*='vendor']"
            )
            ...
            delivery_fee = await self.extract_text(
                page,
                [
                    "[data-testid='delivery-fee'], [class*='delivery']",
                    "span:has-text('$'):near(span:has-text('entrega'))",
                ],
            )
            ...
            est_time = await self.extract_text(
                page,
                [
                    "[data-testid='eta'], [class*='eta'], [class*='time']",
                    "span:has-text('min')",
                ],
            )
    ...
    async def scrape_restaurant(
        ...
        """Scrape restaurant data from Didi Food.
...
async def main():
    ...
    await scraper.run("didi_raw.json")
